...
inline uint64 compute(
    ...
    if(op == 0){	
        ...
        asm volatile("fence");
        ROCC_INSTRUCTION_DSS(0, rd, scalaA, scalaB, 9);// 比较src0 < src1 返回 1
        asm volatile("fence":::"memory"); 
    }
...
uint64 dcompute(
	...
	printf("start compute %s...\n", op==0 ? "add" :(op==1 ? "subtract" :(op==2 ? "product" :(op==3 ? "divide" : (op==4 ? "product-add-sum":"compare")))));
    ...
    if(op == 0){
        ...
        start = rdcycle();	
        asm volatile("fence");
        ROCC_INSTRUCTION_DSS(0, rd, scalaA, scalaB, 9);// 乘加求和
        asm volatile("fence":::"memory");  
        cycles = rdcycle() - start;
    }

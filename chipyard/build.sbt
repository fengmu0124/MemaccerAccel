...
lazy val chipyard = conditionalDependsOn(project in file("generators/chipyard"))
 .dependsOn(boom, hwacha, sifive_blocks, sifive_cache, utilities, iocell,
   ...
   gemmini, icenet, tracegen, ariane, nvdla,dma_stride,memaccer)
 .settings(commonSettings)
...
// zwk add start
// lazy val rocclcm = (project in file("generators/rocc-lcm"))
//   .dependsOn(rocketchip)
//   .settings(commonSettings)

// lazy val roccmem = (project in file("generators/rocc-mem"))
//  .dependsOn(rocketchip)
//  .settings(commonSettings)

// //hassa
// lazy val hasaa = (project in file("generators/hasaa"))
//   .dependsOn(rocketchip, chisel_testers, midasTargetUtils, icenet)
//   .settings(commonSettings)

lazy val dma_stride = (project in file("generators/rocc-dma-stride"))
  .dependsOn(rocketchip, testchipip, utilities, chisel_testers, midasTargetUtils, icenet)
  .settings(commonSettings)

lazy val memaccer = (project in file("generators/memaccer"))
  .dependsOn(rocketchip, testchipip, utilities, chisel_testers, midasTargetUtils, icenet)
  .settings(commonSettings)
// zwk add end
